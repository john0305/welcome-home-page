-- RadarIQ — Market Intelligence Layer: Phase 1
-- New tables, alterations to existing tables, and seeded defaults.
...
-- ─── 2. Platform Settings (admin-adjustable, no deploy needed) ───────────────
...
CREATE POLICY "api_quota_log_admin_all"
  ON public.api_quota_log FOR ALL
  TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());

CREATE INDEX idx_api_quota_log_called_at ON public.api_quota_log(called_at DESC);
CREATE INDEX idx_api_quota_log_user ON public.api_quota_log(user_id, called_at DESC);
...
CREATE POLICY "market_insight_cache_authenticated_read"
  ON public.market_insight_cache FOR SELECT
  TO authenticated USING (true);

CREATE POLICY "market_insight_cache_admin_all"
  ON public.market_insight_cache FOR ALL
  TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());

CREATE INDEX idx_market_insight_cache_expires ON public.market_insight_cache(expires_at);

-- ─── 5. Competitor Snapshots (shared, not per-user) ──────────────────────────
...
CREATE POLICY "competitor_snapshots_authenticated_read"
  ON public.competitor_snapshots FOR SELECT
  TO authenticated USING (true);

CREATE POLICY "competitor_snapshots_admin_all"
  ON public.competitor_snapshots FOR ALL
  TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());

CREATE UNIQUE INDEX idx_competitor_snapshots_unique
  ON public.competitor_snapshots(keyword_cluster, etsy_listing_id, (captured_at::date));
CREATE INDEX idx_competitor_snapshots_cluster ON public.competitor_snapshots(keyword_cluster, captured_at DESC);
CREATE INDEX idx_competitor_snapshots_listing ON public.competitor_snapshots(etsy_listing_id);

-- ─── 6. Listing Market Scores (per-user) ─────────────────────────────────────
...
CREATE POLICY "listing_market_scores_owner_select"
  ON public.listing_market_scores FOR SELECT
  TO authenticated USING (auth.uid() = user_id);

CREATE POLICY "listing_market_scores_admin_all"
  ON public.listing_market_scores FOR ALL
  TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());

CREATE INDEX idx_listing_market_scores_user ON public.listing_market_scores(user_id, scored_at DESC);
CREATE INDEX idx_listing_market_scores_listing ON public.listing_market_scores(listing_id, scored_at DESC);

-- ─── 7. User Niche Profiles ──────────────────────────────────────────────────
...
CREATE POLICY "user_niche_profiles_owner_select"
  ON public.user_niche_profiles FOR SELECT
  TO authenticated USING (auth.uid() = user_id);

CREATE POLICY "user_niche_profiles_admin_all"
  ON public.user_niche_profiles FOR ALL
  TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
...
CREATE POLICY "seed_niches_authenticated_read"
  ON public.seed_niches FOR SELECT
  TO authenticated USING (true);

CREATE POLICY "seed_niches_admin_all"
  ON public.seed_niches FOR ALL
  TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
...
CREATE POLICY "algorithm_weights_authenticated_read"
  ON public.algorithm_weights FOR SELECT
  TO authenticated USING (true);

CREATE POLICY "algorithm_weights_admin_all"
  ON public.algorithm_weights FOR ALL
  TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
...
CREATE POLICY "algorithm_weight_history_admin_all"
  ON public.algorithm_weight_history FOR ALL
  TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());

-- Seed initial active weights
...
CREATE POLICY "action_effectiveness_authenticated_read"
  ON public.action_effectiveness FOR SELECT
  TO authenticated USING (true);

CREATE POLICY "action_effectiveness_admin_all"
  ON public.action_effectiveness FOR ALL
  TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());

-- ─── 11. User Listing Actions ────────────────────────────────────────────────
...
CREATE POLICY "user_listing_actions_owner_select"
  ON public.user_listing_actions FOR SELECT
  TO authenticated USING (auth.uid() = user_id);

CREATE POLICY "user_listing_actions_admin_all"
  ON public.user_listing_actions FOR ALL
  TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());

CREATE INDEX idx_user_listing_actions_user ON public.user_listing_actions(user_id, performed_at DESC);
CREATE INDEX idx_user_listing_actions_listing ON public.user_listing_actions(listing_id, performed_at DESC);
CREATE INDEX idx_user_listing_actions_attribution ON public.user_listing_actions(attribution_window_ends) WHERE reverted_at IS NULL;

-- ─── 12. Niche Health (time-series per cluster) ──────────────────────────────
...
CREATE POLICY "niche_health_authenticated_read"
  ON public.niche_health FOR SELECT
  TO authenticated USING (true);

CREATE POLICY "niche_health_admin_all"
  ON public.niche_health FOR ALL
  TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());

CREATE INDEX idx_niche_health_cluster ON public.niche_health(keyword_cluster, date DESC);

-- ─── 13. Platform Daily Metrics ──────────────────────────────────────────────
...
CREATE POLICY "platform_daily_metrics_admin_all"
  ON public.platform_daily_metrics FOR ALL
  TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());

CREATE INDEX idx_platform_daily_metrics_date ON public.platform_daily_metrics(date DESC);
...
CREATE POLICY "pipeline_run_log_owner_select"
  ON public.pipeline_run_log FOR SELECT
  TO authenticated USING (auth.uid() = user_id OR public.is_platform_admin());

CREATE POLICY "pipeline_run_log_admin_all"
  ON public.pipeline_run_log FOR ALL
  TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());

CREATE INDEX idx_pipeline_run_log_user ON public.pipeline_run_log(user_id, started_at DESC);
CREATE INDEX idx_pipeline_run_log_status ON public.pipeline_run_log(status, started_at DESC);

-- NOTE: echo_conversations is covered by existing chat_messages + chat_sessions tables.
-- No new table needed — the market intelligence layer will query chat_messages directly.

-- ─── 15. Alter existing tables ───────────────────────────────────────────────

-- user_profiles: market intelligence pipeline tracking
...
-- stores: market context + suppression analysis
...
-- listing_versions: revert attribution and tracking
